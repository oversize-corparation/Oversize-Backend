...
exports.default = {
  ...
  CALLBACK_GOOGLE: async function (req, res, next) {
    try {
      ...
      if (!user.verified_email)
        throw new error_1.ClientError(
          "Something went wrong with your gmail account!",
          400
        );
      ...
      if (!isExists) {
        ...
        res.redirect(
          `http://localhost:4000?token=${createToken({
  ...
  REGISTER: async function (req, res, next) {
    try {
      ...
      const otp = await (0, mailer_1.sendOTP)(newUser.email);
      ...
      return res
        .status(200)
        .json({ message: "We send your email identify number", status: 200 });
      // res.status(201).json({
  ...
  LOGIN: async function (req, res, next) {
    try {
      ...
      if (
        isExists.locked_until &&
        (0, dayjs_1.default)().isBefore(isExists.locked_until)
      ) {
        const waitTime = (0, dayjs_1.default)(isExists.locked_until).diff(
          (0, dayjs_1.default)(),
          "second"
        );
        throw new error_1.ClientError(
  ...
  SEND_OTP: async (req, res, next) => {
    try {
      ...
      if (!user) throw new error_1.ClientError("This user not found", 400);
  ...
  VERIFY_OTP: async function (req, res, next) {
    try {
      ...
      const validator = validator_1.verifyOtpValidator.validate({
        email,
        code,
        restoration,
      });
